...
uint8_t Generator::opIndex() const {
    std::discrete_distribution<> opcode_dist(weights.begin(), weights.end());
    int r = opcode_dist(rng);
    return static_cast<uint8_t>(r);
}
int8_t Generator::oprand_full() const {
    int r = oprand_full_dist(rng);
    return static_cast<int8_t>(r);
}
int8_t Generator::oprand_reg() const {
    int r = oprand_reg_dist(rng);
    return static_cast<int8_t>(r);
}
int8_t Generator::oprand_positive() const {
    int r = oprand_positive_dist(rng);
    return static_cast<int8_t>(r);
}
int8_t Generator::oprand_negative() const {
    int r = oprand_negative_dist(rng);
    return static_cast<int8_t>(r);
}
int8_t Generator::oprand_binomial() const {
...
InstructionEx Generator::generateInstruction() const {
    InstructionEx r{};
    int index = opIndex();
    ...
    r.cost = entry.costFunction;
    return r;
}

int8_t Generator::oprand(OprandRnd rnd) const {
    switch (rnd) {
...
ProgramEx Generator::generateProgram() const {
